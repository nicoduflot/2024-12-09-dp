...
Grâce au débug de twig, n inspectant le code, il est possible de trouver les éléments qui sont utilisés dans le thèmes, et le débug nous propose des nuveaux noms pour ces fichiers, qui permettent de surcharger l'apparence des blocs particuliers.

De plus, on est capcable de créer un fichier css `style.css` dans lequel on applique le CSS particulier désiré pour le thème
